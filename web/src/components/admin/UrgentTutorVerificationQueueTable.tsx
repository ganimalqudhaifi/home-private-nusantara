...
export function UrgentTutorVerificationQueueTable({
  ...
  return (
    <div
      className={`bg-white dark:bg-surface-container-low border border-border-whisper dark:border-outline-variant rounded-2xl p-6 shadow-sm space-y-4 ${className}`}
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-border-whisper dark:border-outline-variant">
        <div className="flex items-center gap-2">
          <AlertCircle className="w-5 h-5 text-status-warning shrink-0" />
          <h3 className="font-headline text-base md:text-lg font-bold text-primary dark:text-white">
            Antrean Verifikasi Dokumen & Wawancara Pengajar
          </h3>
        </div>
        <span className="bg-amber-50 text-status-warning dark:bg-amber-950/40 dark:text-amber-300 text-xs font-bold px-2.5 py-1 rounded-full border border-amber-200 dark:border-amber-800 w-fit">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[600px]">
          <thead>
            <tr className="border-b border-border-whisper dark:border-outline-variant text-xs text-text-muted uppercase tracking-wider">
          ...
          <tbody className="divide-y divide-border-whisper dark:divide-outline-variant/60 text-xs">
            {pendingTutors.map((tutor) => (
              <tr key={tutor.id} className="hover:bg-surface-container-low/40 dark:hover:bg-surface-container-high/40 transition-colors">
                <td className="py-4 font-semibold text-primary dark:text-white flex items-center gap-3">
                ...
                <td className="py-4 text-text-primary dark:text-gray-200">{tutor.university}</td>
                ...
                <td className="py-4 text-center">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-300 hover:bg-emerald-100 transition-colors"
