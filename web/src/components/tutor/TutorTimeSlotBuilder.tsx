...
export function TutorTimeSlotBuilder({
  ...
  return (
    <div
      className={`bg-white dark:bg-surface-container-low rounded-2xl border border-border-whisper dark:border-outline-variant overflow-hidden shadow-sm ${className}`}
    >
      <div className="p-6 md:p-8 pb-4 border-b border-border-whisper dark:border-outline-variant">
        <h3 className="font-headline text-lg font-bold text-primary dark:text-white flex items-center gap-2">
          <Calendar className="w-5 h-5 text-primary-container dark:text-blue-300" />
      ...
      {/* Day Selector Tabs */}
      <div className="flex overflow-x-auto border-b border-border-whisper dark:border-outline-variant bg-surface-container-low dark:bg-surface-container-high hide-scrollbar">
        {days.map((day) => {
          ...
          return (
            <button
              ...
              className={`px-6 py-4 font-headline text-sm font-semibold whitespace-nowrap transition-all flex items-center gap-2 border-b-2 ${
                isSelected
                  ? 'border-primary-container text-primary-container dark:text-white dark:border-blue-400 bg-white dark:bg-surface-container-low'
      ...
      <div className="p-6 md:p-8 space-y-4">
        <div className="flex items-center justify-between">
          <h4 className="font-headline text-sm font-bold text-primary dark:text-white">
        ...
        {filteredSlots.length === 0 ? (
          <div className="py-8 text-center border-2 border-dashed border-border-whisper dark:border-outline-variant rounded-xl bg-surface-container-low/30">
                ...
                <div
                  ...
                  className={`flex items-center justify-between p-3.5 rounded-xl border transition-all ${
                    isBooked
                      ...
                      : 'bg-white dark:bg-surface-container-lowest border-border-whisper dark:border-outline-variant hover:border-primary-container'
                  }`}
                >
                  ...
                  {!isBooked ? (
                    <button
                      ...
                      className="p-1.5 rounded-lg text-text-muted hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/40 transition-colors"
                      title="Hapus Slot"
                    >
                      <Trash2 className="w-4 h-4" />
                    </button>
                  ) : (
                    <span className="text-xs text-text-muted italic px-2 py-1 bg-white/60 dark:bg-black/30 rounded-md">
                      Terkunci
                    </span>
                  )}
                </div>
        ...
        {showAddForm ? (
          <form
            onSubmit={handleAdd}
            className="p-4 rounded-xl border border-primary-container/30 bg-blue-50/30 dark:bg-blue-950/20 space-y-3"
          >
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  className="px-3 py-2 rounded-lg border border-border-whisper bg-white dark:bg-surface-container-lowest text-xs font-mono"
                  required
                />
              </div>
              <div className="flex flex-col gap-1">
          ...
          <button
            ...
            className="w-full py-3 border-2 border-dashed border-border-whisper dark:border-outline-variant hover:border-primary-container dark:hover:border-blue-400 rounded-xl text-primary dark:text-blue-300 font-headline text-xs font-bold transition-all flex items-center justify-center gap-2 hover:bg-surface-container-low"
          >
