...
export function TutorVerificationSteps({
  ...
  return (
    <div
      className={`bg-white dark:bg-surface-container-low rounded-2xl p-6 md:p-8 border border-border-whisper dark:border-outline-variant shadow-sm ${className}`}
    >
      <h3 className="font-headline text-lg font-bold text-primary dark:text-white mb-6">
        Status Verifikasi Pendaftaran
      </h3>

      <div className="relative">
        {/* Vertical line indicator */}
        <div className="absolute left-5 top-5 bottom-8 w-0.5 bg-border-whisper dark:bg-outline-variant z-0" />
              ...
              <div key={s.stepNumber} className="flex gap-4 items-start">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 transition-all ${
                    isCompleted
                      ? 'bg-emerald-50 text-emerald-600 border-emerald-500 dark:bg-emerald-950/40 dark:text-emerald-300'
                      : isInProgress
                ...
                <div className={`flex-1 ${!isCompleted && !isInProgress ? 'opacity-50' : ''}`}>
                  <div className="flex items-center gap-2">
                    <h4 className="font-headline text-sm md:text-base font-bold text-primary dark:text-white">
                      {s.title}
                    </h4>
                    {isInProgress && (
                  ...
                  <p className="text-xs md:text-sm text-text-muted dark:text-gray-400 mt-1 leading-relaxed">
                    {s.description}
                  </p>
                </div>
              </div>
