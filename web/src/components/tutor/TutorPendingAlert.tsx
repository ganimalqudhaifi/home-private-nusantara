...
export function TutorPendingAlert({ className = '' }: TutorPendingAlertProps) {
  return (
    <div
      className={`bg-white dark:bg-surface-container-low rounded-2xl p-6 border-l-4 border-l-status-warning border border-border-whisper dark:border-outline-variant shadow-sm relative overflow-hidden ${className}`}
    >
      ...
      <div className="flex flex-col sm:flex-row items-start gap-4 relative z-10">
        <div className="w-10 h-10 rounded-xl bg-amber-50 dark:bg-amber-950/40 text-status-warning flex items-center justify-center shrink-0">
          <AlertCircle className="w-6 h-6" />
        </div>

        <div className="flex-1">
          <h2 className="font-headline text-lg md:text-xl font-bold text-primary dark:text-white mb-1.5">
            Akun Anda Sedang Dalam Proses Verifikasi Admin
          </h2>
          <p className="text-sm text-text-muted dark:text-gray-300 leading-relaxed max-w-2xl">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <a
              href={`https://wa.me/${BRAND_INFO.contact.whatsapp}?text=Halo%20Admin%20Home%20Private%20Nusantara,%20saya%20ingin%20menanyakan%20status%20verifikasi%20tutor%20saya.`}
              target="_blank"
              rel="noreferrer"
