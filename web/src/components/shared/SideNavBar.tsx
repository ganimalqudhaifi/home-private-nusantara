...
export function SideNavBar({ role, className = '' }: SideNavBarProps) {
  ...
  return (
    <aside
      className={`bg-surface-container-lowest dark:bg-surface-container-low border-r border-border-whisper dark:border-outline-variant w-64 flex flex-col p-4 shrink-0 min-h-screen sticky top-0 ${className}`}
    >
      {/* Brand Header */}
      <div className="mb-6 px-3 py-2">
        <Link href="/" className="flex items-center gap-3 group">
          <div className="relative w-8 h-8 rounded-lg overflow-hidden border border-border-whisper dark:border-outline-variant shrink-0 bg-white">
            <Image
          ...
          <div className="flex flex-col">
            <h1 className="font-headline text-base font-bold text-primary dark:text-white tracking-tight">
              Home Private
            </h1>
            <p className="text-[11px] text-text-muted dark:text-gray-400 font-medium capitalize">
              Portal {role}
            </p>
          </div>
        </Link>
            ...
            <Link
              ...
              className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl font-body text-sm font-medium transition-all ${
                isActive
                  ...
                  : 'text-on-surface-variant hover:bg-surface-container-high dark:text-gray-300 dark:hover:bg-surface-container-highest'
              }`}
            >
      ...
      {/* Footer / CTA Actions */}
      <div className="mt-auto pt-4 border-t border-border-whisper dark:border-outline-variant space-y-2">
        {role === 'student' && (
          <Link
            href="/student/search"
        ...
        <Link
          href="/auth"
          className="flex items-center gap-3 px-3.5 py-2 rounded-xl text-text-muted hover:text-[#DC2626] hover:bg-red-50 dark:hover:bg-red-950/20 text-xs font-medium transition-colors"
        >
