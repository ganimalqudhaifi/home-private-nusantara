    ...
    <section id="testimonials" className={`w-full py-16 md:py-24 px-4 md:px-8 max-w-7xl mx-auto ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {TESTIMONIALS.map((t, idx) => (
          <div
            key={idx}
            className="bg-white dark:bg-surface-container-low border border-border-whisper dark:border-outline-variant p-6 rounded-2xl shadow-xs flex flex-col justify-between hover:shadow-md transition-shadow"
          >
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <Quote className="w-5 h-5 text-gray-300 dark:text-gray-600" />
              </div>
              <p className="text-sm text-text-primary dark:text-gray-200 leading-relaxed italic mb-6">
                &ldquo;{t.comment}&rdquo;
              </p>
            </div>

            <div className="flex items-center gap-3 pt-4 border-t border-border-whisper dark:border-outline-variant">
              ...
              <div className="flex flex-col min-w-0">
                ...
                <p className="text-xs text-text-muted dark:text-gray-400 truncate">{t.role}</p>
