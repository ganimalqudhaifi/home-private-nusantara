...
export function StudentSessionsTabs({
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      {/* Tab Switcher */}
      <div className="flex border-b border-border-whisper dark:border-outline-variant gap-8">
        <button
          type="button"
          onClick={() => setActiveTab('upcoming')}
          className={`pb-3 font-headline text-base font-bold transition-all border-b-2 flex items-center gap-2 ${
            activeTab === 'upcoming'
              ? 'border-primary-container text-primary-container dark:text-blue-300 dark:border-blue-400'
      ...
      <div className="space-y-4">
        {activeTab === 'upcoming' ? (
          upcomingSessions.length === 0 ? (
            <div className="p-12 text-center bg-white dark:bg-surface-container-low rounded-2xl border border-border-whisper dark:border-outline-variant space-y-3">
          ...
          <div className="p-12 text-center bg-white dark:bg-surface-container-low rounded-2xl border border-border-whisper dark:border-outline-variant">
