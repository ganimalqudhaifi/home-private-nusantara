...
- **Secondary Accent**: `#D97706` (Warm Amber - active day pills & time tags)
- **Active State Fill**: `#EFF6FF` (Soft Blue Tint with `border-blue-600`)
- **Alert / Warning**: `#E11D48` (Rose Crimson for active booking collision warning)
...
1. **Header & Status Toggle Section**:
   ...
   - Master Toggle Ketersediaan: `[Status: Aktif Menerima Siswa Baru ✓]` (Switch toggle On/Off).
...
4. **Bagian 3: Proteksi & Peringatan Jadwal Aktif (Edge Case Modal / Inline Alert)**:
   - Jika pengajar mencoba menonaktifkan hari yang sudah memiliki pesanan aktif:
     - Banner Peringatan Merah: "⚠️ Peringatan: Hari Senin memiliki 1 sesi aktif terkonfirmasi bersama Siswa Fajar Pratama. Anda tidak dapat menonaktifkan hari ini sebelum sesi diselesaikan atau dikoordinasikan ulang dengan Admin."
...
5. **Sticky Bottom Action Bar**:
   ...
   - Tombol Utama: "Simpan Pengaturan Ketersediaan" (Icon: Save, bg-blue-900 text-white)
...
- Primary Accent: Nusantara Deep Blue (#1E3A8A) for save actions and active selection borders.
- Secondary Accent: Warm Amber (#D97706) for active time slot tags and badge chips.
- Typography: Outfit (Headings) and Geist (Labels, 24-hour time slots, checkboxes).

PAGE STRUCTURE:
1. HEADER: Page title "Pengaturan Jenjang & Slot Ketersediaan Mengajar", description subtitle, and an active availability status toggle switch (On: "Menerima Pesanan").
2. SECTION 1 - TEACHING GRADE MATRIX:
   - Card A (Jenjang SD): Checkbox header "Jenjang Sekolah Dasar (SD)", with 6 selectable class pills (Kelas 1, Kelas 2, Kelas 3, Kelas 4, Kelas 5, Kelas 6) all checked with active blue border.
...
3. SECTION 2 - WEEKLY AVAILABILITY SCHEDULER:
   ...
   - Detailed Day Schedule Panel (showing active days e.g., Senin & Sabtu):
     ...
     - Red lock badge on slots with active student bookings preventing accidental deletion.
4. SECTION 3 - ACTIVE BOOKING COLLISION NOTICE:
   - An alert banner explaining that days with active student reservations cannot be disabled without admin coordination.
5. STICKY BOTTOM BAR: A floating bottom bar with a summary count ("4 Hari Aktif • 9 Slot Jam Tersedia") and a primary blue button "Simpan Pengaturan Ketersediaan".
