...
- Logo Home Private Nusantara dengan teks "Portal Akses Layanan Les Privat SD & SMP".
...
- **Tombol Aksi**: "Masuk ke Portal" (Primary Nusantara Blue `#1E3A8A`).
...
- **Segmentasi Jenjang Pendidikan (Radio Card Selector)**:
  - *Pilihan 1*: `Jenjang SD` (Sekolah Dasar)
  - *Pilihan 2*: `Jenjang SMP` (Sekolah Menengah Pertama)
- **Pilihan Sub-Kelas Dinamis (Dropdown / Pill Selector)**:
...
- **Tombol Aksi**: "Daftar Akun Siswa & Mulai Cari Pengajar" (Primary Blue).
...
- **Notice Box Info Kurasi (Amber Tint)**:
...
- **Tombol Aksi**: "Kirim Pendaftaran Pengajar" (Primary Blue).
...
- Background: Slate Canvas (#F8FAFC) with a subtle educational background pattern.
- Surface: Pure White (#FFFFFF) card with 1px border (#E2E8F0) and smooth elevation shadow.
- Primary Accent: Nusantara Deep Blue (#1E3A8A) for active tabs and primary buttons.
- Secondary Accent: Warm Amber (#D97706) for info notices and pending warnings.
- Text Headings: Charcoal Slate (#0F172A). Text Labels: Slate-700 (#334155).
- Typography: Outfit (Headings) and Geist (Form labels, inputs).

PAGE STRUCTURE:
1. HEADER: Centered logo with emblem "Home Private Nusantara" and sub-caption "Portal Akses Layanan Les Privat SD & SMP".
2. SEGMENTED TAB SWITCHER: 3 equal tab pills:
   - Tab 1: "Masuk Akun" (Active State with dark blue fill and white text)
...
3. STATE A - LOGIN FORM:
   ...
   - Large primary button "Masuk ke Portal".
4. STATE B - STUDENT REGISTRATION FORM (When Tab 2 clicked):
   ...
   - Primary button "Daftar Akun Siswa".
5. STATE C - TUTOR REGISTRATION FORM (When Tab 3 clicked):
   ...
   - Amber alert info card explaining: "Akun pengajar berstatus PENDING hingga lolos verifikasi berkas & wawancara offline oleh Admin."
   - Primary button "Kirim Pendaftaran Pengajar".
